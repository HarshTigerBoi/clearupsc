...
import { ArrowLeft, ArrowRight, BookOpen, CheckCircle2, ChevronRight, ExternalLink, FileText } from "lucide-react";
...
const LazyArsenal = lazy(() => import("@/components/study/Arsenal"));
const LazyQuickRecall = lazy(() => import("@/components/study/QuickRecall"));
const LazyQuestionPractice = lazy(() => import("@/components/study/QuestionPractice"));
...
export default function StudyTopicClient({ params }: { params: { topicId: string } }) {
  ...
  const connectedTopics = data.notesStructured.connected_topics?.length ? data.notesStructured.connected_topics : data.notesStructured.mindmap.branches;
        ...
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[160px_minmax(0,1fr)] lg:px-8">
          ...
          <div className="min-w-0 space-y-8">
            ...
            <StepSection id="quick-recall" step="03.5" label="03.5 QUICK RECALL" title="Recall Before You Read Again" tone="white">
              <LazyStepMount label="Quick Recall">
                <Suspense fallback={<LazyStepSkeleton label="Quick Recall" />}>
                  <LazyQuickRecall topicKey={params.topicId} conciseNotes={data.notesStructured.concise_notes} />
                </Suspense>
              </LazyStepMount>
            </StepSection>
            ...
            <StepSection id="arsenal" step="05" label="Arsenal" title="Exam Warrior Layer" tone="gray">
              <LazyStepMount label="Arsenal">
                <Suspense fallback={<LazyStepSkeleton label="Arsenal" />}>
                  <LazyArsenal
...
function LazyStepSkeleton({ label }: { label: string }) {
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="h-4 w-32 animate-pulse rounded-full bg-slate-200" />
      <div className="mt-4 h-20 animate-pulse rounded-2xl bg-slate-100" />
      <p className="mt-4 text-sm font-bold text-slate-500">Loading {label}...</p>
    </div>
  );
}

function LazyStepMount({ label, children }: { label: string; children: ReactNode }) {
  ...
  return <div ref={ref}>{visible ? children : <LazyStepPreview label={label} />}</div>;
}

function LazyStepPreview({ label }: { label: string }) {
  return (
    <div className="rounded-3xl border border-dashed border-slate-300 bg-white/70 p-5 text-sm font-bold text-slate-500 shadow-sm">
      {label} loads when you reach this step, keeping the first screen fast.
    </div>
  );
}
